...
public class Trees_6 {
    private static <E extends Comparable<E>> int depth(BNode<E> node, E target) {
        int d = 1;
        while(node != null) {
            if(target.compareTo(node.info) == 0) return d;
            else if(target.compareTo(node.info) < 0) node = node.left;
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < n + q; i++) {
            ...
            if(parts[0].equals("insert")) {
                ...
                System.out.println(depth(bst.getRoot(), val));
            }
        }
    }
}
